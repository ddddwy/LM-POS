...
parser.add_argument('--trainfname', type=str, default='train_0.txt',
                    help='name of the training file')
parser.add_argument('--validfname', type=str, default='valid_0.txt',
                    help='name of the validation file')
parser.add_argument('--testfname', type=str, default='test_0.txt',
...
def test_get_batch(sent_ids, tag_ids, evaluation=False):
    ...
    if args.cuda:
        return input_token.cuda(), output_token.cuda(), input_tag.cuda()
    else:
...
def test_evaluate(test_lm_sentences, lm_data_source, ccg_data_source):
    ...
    for i in range(len(lm_data_source)):
        ...
        input_token, output_token, input_tag = test_get_batch(sent_ids, tag_ids, evaluation=True)
        input_token = input_token.unsqueeze(1)  # [seq_len, 1] 
        output, hidden = model(input_token, hidden)
        output_flat = output.view(-1, ntokens)
        curr_loss = criterion(output_flat, output_token).item()
...
def evaluate(lm_data_source, ccg_data_source):
    ...
    for i in range(lm_data_source.size(1)):
        input_token, output_token, input_tag = get_batch(lm_data_source, ccg_data_source, i)
        output, hidden = model(input_token, hidden)
        output_flat = output.view(-1, ntokens)
        curr_loss = len(data) * criterion(output_flat, output_token).data
        total_loss += curr_loss
        hidden = repackage_hidden(hidden)
    return total_loss / lm_data_source.size(1)


def train(train_lm_data, train_ccg_data):
...
if not args.test:
    try:
        for epoch in range(1, args.epochs+1):
            ...
            train(train_lm_data, train_ccg_data)
    ...
    with open(args.save, 'rb') as f:
        model = torch.load(f)
    # Run on test data.
    test_loss = test_evaluate(test_lm_sentences, test_lm_data, test_ccg_data)
    print('=' * 89)
